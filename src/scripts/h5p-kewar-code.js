...
export default class KewArCode extends H5P.EventDispatcher {
  ...
  constructor(params) {
    ...
    this.on('resize', () => {
      ...
      const heightSVG = (this.codeSVG) ?  this.codeSVG.getBoundingClientRect().height : 0;
    ...
    this.attach = function ($wrapper) {
      ...
      // Adjust SVG generated by qrcode-generator
      this.codeSVG = this.qrcodeContainer.querySelector('svg');
      // Make SVG use full width up to max-width
      this.codeSVG.removeAttribute('width');
      this.codeSVG.removeAttribute('height');

      if (this.params.behaviour.maxSize) {
        this.codeSVG.style.maxWidth = this.params.behaviour.maxSize;
        this.codeSVG.style.maxHeight = this.params.behaviour.maxSize;
      }

      // Apply custom colors
      const codePath = this.codeSVG.querySelector('path');
      codePath.setAttribute('fill', this.params.behaviour.codeColor);

      const codeRect = this.codeSVG.querySelector('rect');
    ...
    /**
     * Set the SVG scale explicitly.
     * Required by IE11 only. Remove once IE11 support runs out.
     */
    this.scaleSVG = function () {
      const sizeContainer = this.qrcodeContainer.offsetWidth;
      this.codeSVG.style.width = `${sizeContainer}px`;
      this.codeSVG.style.height = `${sizeContainer}px`;

      // Wait for container to be resized and get initial width
      if (!this.foo) {
        this.foo = true;
        setTimeout(() => {
          this.trigger('resize');
